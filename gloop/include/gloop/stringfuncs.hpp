...
template <typename _CharT, typename _Traits, typename _Alloc>
static inline bool endswith(const std::basic_string<_CharT, _Traits, _Alloc>& a,
                            const _CharT* b) {
...
static inline std::basic_string<_CharT, _Traits, _Alloc> lstrip(
  ...
  std::locale loc;
  while (p < a.size() && std::isspace(a[p], loc))
    p++;
  if (p == a.size())
    return "";
  return a.substr(p);
...
static inline std::basic_string<_CharT, _Traits, _Alloc> strip(
  ...
  while (q > p && std::isspace(a[q - 1], loc))
    q--;
  return a.substr(p, q - p);
}
