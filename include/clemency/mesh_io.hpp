...
struct vertex : public gloop::stream::null_reader {
  ...
  virtual void next() { mesh->add_vertex(v3d_t::zero()); }
  virtual void length(int l) {
    if (l > 0)
      mesh->vertices.reserve(mesh->vertices.size() + l);
  }
  virtual void end() {}
  typename mesh_t::vert_t& curr() const { return mesh->vertices.back(); }

  template <int idx>
  struct component : public gloop::stream::reader<double> {
    const vertex* i;
    component(const vertex* _i) : i(_i) {}
    virtual void value(double val) { i->curr().pos.v[idx] = val; }
  };
};
...
struct face : public gloop::stream::null_writer {
  ...
  struct idx : public gloop::stream::writer<size_t> {
    ...
    virtual size_t value() { return f->v[i++]; }
  };
};
}
}
...
bool read_mesh(std::istream& in, filetype_t& f, mesh_t*& mesh) {
  ...
  f.addReader("polyhedron.vertex.x",
              new typename vertex_t::template component<0>(vi));
  f.addReader("polyhedron.vertex.y",
              new typename vertex_t::template component<1>(vi));
  f.addReader("polyhedron.vertex.z",
              new typename vertex_t::template component<2>(vi));

  f.addReader("polyhedron.face.vertex_indices",
              new detail::read::face_idx<mesh_t>(mesh));

  f.addReader("polyhedron.tristrips.vertex_indices",
              new detail::read::tristrip_idx<mesh_t>(mesh));
...
bool write_mesh(std::ostream& out, filetype_t& f, const mesh_t* mesh) {
  ...
  f.addWriter("polyhedron.vertex.x",
              new typename vertex_t::template component<0>(*vi));
  f.addWriter("polyhedron.vertex.y",
              new typename vertex_t::template component<1>(*vi));
  f.addWriter("polyhedron.vertex.z",
              new typename vertex_t::template component<2>(*vi));

  face_t* fi = new face_t(mesh);
  f.addWriter("polyhedron.face", fi);
  f.addWriter("polyhedron.face.vertex_indices",
              new typename face_t::idx(
                  *fi, gloop::stream::smallest_type(mesh->vertices.size())));
