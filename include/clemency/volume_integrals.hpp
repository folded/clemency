...
// This code is derived from http://www.astro.cornell.edu/~carcich/LRO/feature/test/homegrown/misc/hypspin/volInt.c
...
struct volint_t {
  ...
  void _compute_projection_integrals(const mesh_t *mesh, const typename mesh_t::tri_t *t) {
    double a0, a1, da;
    double b0, b1, db;
  ...
  void _compute_face_integrals(const mesh_t *mesh, const typename mesh_t::tri_t *t, const v3d_t &norm, const double w) {
    double k1, k2, k3, k4;

    _compute_projection_integrals(mesh, t);

    k1 = 1 / norm.v[C]; k2 = k1 * k1; k3 = k2 * k1; k4 = k3 * k1;
    ...
    Faa = k1 * Paa;
    Fbb = k1 * Pbb;
    Fcc = k3 * (SQR(norm.v[A])*Paa + 2*norm.v[A]*norm.v[B]*Pab + SQR(norm.v[B])*Pbb
                + w*(2*(norm.v[A]*Pa + norm.v[B]*Pb) + w*P1));

    Faaa = k1 * Paaa;
    Fbbb = k1 * Pbbb;
    Fccc = -k4 * (CUBE(norm.v[A])*Paaa + 3*SQR(norm.v[A])*norm.v[B]*Paab 
                  + 3*norm.v[A]*SQR(norm.v[B])*Pabb + CUBE(norm.v[B])*Pbbb
                  + 3*w*(SQR(norm.v[A])*Paa + 2*norm.v[A]*norm.v[B]*Pab + SQR(norm.v[B])*Pbb)
                  + w*w*(3*(norm.v[A]*Pa + norm.v[B]*Pb) + w*P1));

    Faab = k1 * Paab;
    Fbbc = -k2 * (norm.v[A]*Pabb + norm.v[B]*Pbbb + w*Pbb);
    Fcca = k3 * (SQR(norm.v[A])*Paaa + 2*norm.v[A]*norm.v[B]*Paab + SQR(norm.v[B])*Pabb
                 + w*(2*(norm.v[A]*Paa + norm.v[B]*Pab) + w*Pa));
  }

  template<typename iter_t>
  void _compute_volume_integrals(const mesh_t *mesh, iter_t fbegin, iter_t fend) {
    T0 = 0.0;
    T1 = v3d_t::zero();
    T2 = v3d_t::zero();
    TP = v3d_t::zero();
