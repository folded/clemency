...
#define _check(status)                                                        \
  do {                                                                        \
    __check(status, std::string(__PRETTY_FUNCTION__) + " " + __FILE__ + ":" + \
                        _xstr(__LINE__));                                     \
  } while (0)
...
struct info_traits_t<cl_device_id> {
  ...
  typedef cl_int (
      *info_func_t)(cl_device_id, cl_device_info, size_t, void*, size_t*);
...
struct info_traits_t<cl_platform_id> {
  ...
  typedef cl_int (
      *info_func_t)(cl_platform_id, cl_platform_info, size_t, void*, size_t*);
...
struct info_traits_t<cl_context> {
  ...
  typedef cl_int (
      *info_func_t)(cl_context, cl_context_info, size_t, void*, size_t*);
...
struct info_traits_t<cl_command_queue> {
  ...
  typedef cl_int (*info_func_t)(cl_command_queue,
...
struct info_traits_t<cl_program> {
  ...
  typedef cl_int (
      *info_func_t)(cl_program, cl_program_info, size_t, void*, size_t*);
...
struct info_traits_t<cl_kernel> {
  ...
  typedef cl_int (
      *info_func_t)(cl_kernel, cl_kernel_info, size_t, void*, size_t*);
...
struct info_traits_t<cl_event> {
  ...
  typedef cl_int (
      *info_func_t)(cl_event, cl_event_info, size_t, void*, size_t*);
...
struct info_traits_t<std::pair<cl_program, cl_device_id> > {
  ...
  typedef cl_int (*info_func_t)(std::pair<cl_program, cl_device_id>,
                                cl_program_build_info,
                                size_t,
                                void*,
                                size_t*);

  static inline cl_int _clGetProgramBuildInfo(
      std::pair<cl_program, cl_device_id> id,
      cl_program_build_info info,
      size_t param_sz,
      void* param_val,
      size_t* param_sz_ret) {
    return clGetProgramBuildInfo(id.first, id.second, info, param_sz, param_val,
                                 param_sz_ret);
  }
...
struct info_t {
  static inline cl_int _call(id_type_t id,
                             ...
                             void* param_val,
                             size_t* param_sz_ret) {
    return info_traits_t<id_type_t>::func()(id, info, param_sz, param_val,
                                            param_sz_ret);
  }

  static inline std::string getstr(
      id_type_t id,
      typename info_traits_t<id_type_t>::info_type info) {
    size_t len;
...
struct cmd_rect {
  ...
  derived& rect(size_t x, size_t y, size_t z = 0) {
    v_region[0] = x;
    v_region[1] = y;
    v_region[2] = z;
    return *this;
  }
  derived& src(size_t x, size_t y, size_t z = 0) {
    v_src_origin[0] = x;
    v_src_origin[1] = y;
    v_src_origin[2] = z;
    return *this;
  }
  derived& dst(size_t x, size_t y, size_t z = 0) {
    v_dst_origin[0] = x;
    v_dst_origin[1] = y;
    v_dst_origin[2] = z;
    return *this;
  }
  derived& src_pitch(size_t x, size_t y = 0) {
    src_row_pitch = x;
    src_slice_pitch = y;
    return *this;
  }
  derived& dst_pitch(size_t x, size_t y = 0) {
    dst_row_pitch = x;
    dst_slice_pitch = y;
    return *this;
  }
...
struct cmd_read_rect : public cmd_rect<cmd_read_rect> {
  ...
  cl_int operator()(cl_command_queue queue,
                    size_t n_wait,
                    const cl_event* wait,
                    cl_event* event) const {
    return clEnqueueReadBufferRect(queue, buffer, blocking, v_src_origin,
                                   v_dst_origin, v_region, src_row_pitch,
                                   src_slice_pitch, dst_row_pitch,
                                   dst_slice_pitch, ptr, n_wait, wait, event);
  }
};
...
struct cmd_write_rect : public cmd_rect<cmd_write_rect> {
  ...
  cl_int operator()(cl_command_queue queue,
                    ...
                    const cl_event* wait,
                    cl_event* event) const {
    return clEnqueueWriteBufferRect(queue, buffer, blocking, v_dst_origin,
                                    v_src_origin, v_region, dst_row_pitch,
                                    dst_slice_pitch, src_row_pitch,
                                    src_slice_pitch, ptr, n_wait, wait, event);
  }
};
...
struct cmd_copy_rect : public cmd_rect<cmd_copy_rect> {
  ...
  cl_int operator()(cl_command_queue queue,
                    ...
                    const cl_event* wait,
                    cl_event* event) const {
    return clEnqueueCopyBufferRect(queue, src_buffer, dst_buffer, v_src_origin,
                                   v_dst_origin, v_region, src_row_pitch,
                                   src_slice_pitch, dst_row_pitch,
                                   dst_slice_pitch, n_wait, wait, event);
  }
};
...
struct cmd_read_buffer {
  ...
  cl_int operator()(cl_command_queue queue,
                    ...
                    const cl_event* wait,
                    cl_event* event) const {
    return clEnqueueReadBuffer(queue, buffer, blocking, offset, size, ptr,
                               n_wait, wait, event);
  }
};
...
struct cmd_write_buffer {
  ...
  cl_int operator()(cl_command_queue queue,
                    ...
                    const cl_event* wait,
                    cl_event* event) const {
    return clEnqueueWriteBuffer(queue, buffer, blocking, offset, size, ptr,
                                n_wait, wait, event);
  }
};
...
struct cmd_copy_buffer {
  ...
  cl_int operator()(cl_command_queue queue,
                    ...
                    const cl_event* wait,
                    cl_event* event) const {
    return clEnqueueCopyBuffer(queue, src_buffer, dst_buffer, src_offset,
                               dst_offset, cb, n_wait, wait, event);
  }
};
...
struct cmd_task {
  ...
  cmd_task(cl_kernel _kernel)
      : kernel(_kernel), v_offset(), v_wrk_sz(), v_grp_sz() {}

  cmd_task& offset(size_t x) {
    v_offset.resize(1);
    v_offset[0] = x;
    return *this;
  }
  cmd_task& offset(size_t x, size_t y) {
    v_offset.resize(2);
    v_offset[0] = x;
    v_offset[1] = y;
    return *this;
  }
  cmd_task& offset(size_t x, size_t y, size_t z) {
    v_offset.resize(3);
    v_offset[0] = x;
    v_offset[1] = y;
    v_offset[2] = z;
  ...
  cmd_task& offset(const std::vector<size_t>& v) {
    v_offset = v;
    return *this;
  }

  cmd_task& wrk_sz(size_t x) {
    v_wrk_sz.resize(1);
    v_wrk_sz[0] = x;
    return *this;
  }
  cmd_task& wrk_sz(size_t x, size_t y) {
    v_wrk_sz.resize(2);
    v_wrk_sz[0] = x;
    v_wrk_sz[1] = y;
    return *this;
  }
  cmd_task& wrk_sz(size_t x, size_t y, size_t z) {
    v_wrk_sz.resize(3);
    v_wrk_sz[0] = x;
    v_wrk_sz[1] = y;
    v_wrk_sz[2] = z;
  ...
  cmd_task& wrk_sz(const std::vector<size_t>& v) {
    v_wrk_sz = v;
    return *this;
  }

  cmd_task& grp_sz(size_t x) {
    v_grp_sz.resize(1);
    v_grp_sz[0] = x;
    return *this;
  }
  cmd_task& grp_sz(size_t x, size_t y) {
    v_grp_sz.resize(2);
    v_grp_sz[0] = x;
    v_grp_sz[1] = y;
    return *this;
  }
  cmd_task& grp_sz(size_t x, size_t y, size_t z) {
    v_grp_sz.resize(3);
    v_grp_sz[0] = x;
    v_grp_sz[1] = y;
    v_grp_sz[2] = z;
  ...
  cmd_task& grp_sz(const std::vector<size_t>& v) {
    v_grp_sz = v;
    return *this;
  }
  ...
  cl_int operator()(cl_command_queue queue,
    ...
    if (!v_wrk_sz.size()) {
      ...
      const size_t* global_work_offset =
          (v_offset.size() == n_dim) ? &v_offset[0] : NULL;
      const size_t* local_work_size =
          (v_grp_sz.size() == n_dim) ? &v_grp_sz[0] : NULL;
      return clEnqueueNDRangeKernel(queue, kernel, n_dim, global_work_offset,
                                    global_work_size, local_work_size, n_wait,
                                    wait, event);
    }
  }
};
...
struct dev_t {
  ...
  cl_device_type type() const {
    return info_type::get<cl_device_type>(id, CL_DEVICE_TYPE);
  }
  cl_uint vendor_id() const {
    return info_type::get<cl_uint>(id, CL_DEVICE_VENDOR_ID);
  }
  cl_uint max_compute_units() const {
    return info_type::get<cl_uint>(id, CL_DEVICE_MAX_COMPUTE_UNITS);
  }
  cl_uint max_workitem_dim() const {
    return info_type::get<cl_uint>(id, CL_DEVICE_MAX_WORK_ITEM_DIMENSIONS);
  }
  size_t max_workgroup_size() const {
    return info_type::get<size_t>(id, CL_DEVICE_MAX_WORK_GROUP_SIZE);
  }
  cl_uint max_clockfreq_mhz() const {
    return info_type::get<cl_uint>(id, CL_DEVICE_MAX_CLOCK_FREQUENCY);
  }

  cl_uint global_mem_size() const {
    return info_type::get<cl_uint>(id, CL_DEVICE_GLOBAL_MEM_SIZE);
  }
  cl_uint local_mem_size() const {
    return info_type::get<cl_uint>(id, CL_DEVICE_LOCAL_MEM_SIZE);
  }
  cl_uint max_alloc() const {
    return info_type::get<cl_uint>(id, CL_DEVICE_MAX_MEM_ALLOC_SIZE);
  }

  cl_bool has_image_support() const {
    return info_type::get<cl_bool>(id, CL_DEVICE_IMAGE_SUPPORT);
  }
  cl_bool device_available() const {
    return info_type::get<cl_bool>(id, CL_DEVICE_AVAILABLE);
  }
  cl_bool compiler_available() const {
    return info_type::get<cl_bool>(id, CL_DEVICE_COMPILER_AVAILABLE);
  }

  size_t max_parameter_size() const {
    return info_type::get<size_t>(id, CL_DEVICE_MAX_PARAMETER_SIZE);
  }
  cl_uint max_constant_args() const {
    return info_type::get<cl_uint>(id, CL_DEVICE_MAX_CONSTANT_ARGS);
  }
  ...
  std::string version() const {
    return info_type::getstr(id, CL_DEVICE_VERSION);
  }
  std::string c_version() const {
    return info_type::getstr(id, CL_DEVICE_OPENCL_C_VERSION);
  }
  std::string driver_version() const {
    return info_type::getstr(id, CL_DRIVER_VERSION);
  }
  std::string profile() const {
    return info_type::getstr(id, CL_DEVICE_PROFILE);
  }
  std::string extensions() const {
    return info_type::getstr(id, CL_DEVICE_EXTENSIONS);
  }
  ...
  std::vector<cl_uint> max_workitem_sizes() const {
    ...
    info_type::getvec<cl_uint>(id, CL_DEVICE_MAX_WORK_ITEM_SIZES, &sizes[0],
                               sizes.size());
    return sizes;
  }
...
struct platform_t {
  ...
  platform_t(cl_platform_id _id) : id(_id) {
    ...
    for (cl_uint i = 0; i < n_dev; ++i)
      cl_devices.push_back(new dev_t(dev[i]));
    delete[] dev;
  }
  ...
  std::string profile() const {
    return info_type::getstr(id, CL_PLATFORM_PROFILE);
  }
  std::string version() const {
    return info_type::getstr(id, CL_PLATFORM_VERSION);
  }
  std::string name() const { return info_type::getstr(id, CL_PLATFORM_NAME); }
  std::string vendor() const {
    return info_type::getstr(id, CL_PLATFORM_VENDOR);
  }
  std::string extensions() const {
    return info_type::getstr(id, CL_PLATFORM_EXTENSIONS);
  }
...
struct event_t {
  ...
  event_t(const event_t& event) : id(0) { *this = event; }
  ...
  cl_uint refcount() const {
    return info_type::get<cl_uint>(id, CL_EVENT_REFERENCE_COUNT);
  }
  cl_context context() const {
    return info_type::get<cl_context>(id, CL_EVENT_CONTEXT);
  }
  cl_command_queue queue() const {
    return info_type::get<cl_command_queue>(id, CL_EVENT_COMMAND_QUEUE);
  }
  cl_command_type type() const {
    return info_type::get<cl_command_type>(id, CL_EVENT_COMMAND_TYPE);
  }
  cl_int status() const {
    return info_type::get<cl_int>(id, CL_EVENT_COMMAND_EXECUTION_STATUS);
  }
...
struct mem_t {
  ...
  mem_t(const mem_t& mem) : id(0) { *this = mem; }

  mem_t(cl_mem _id) : id(_id) { clRetainMemObject(id); }
  ...
  cl_uint refcount() const {
    return info_type::get<cl_uint>(id, CL_MEM_REFERENCE_COUNT);
  }
  cl_mem_object_type type() const {
    return info_type::get<cl_mem_object_type>(id, CL_MEM_TYPE);
  }
  cl_mem_flags flags() const {
    return info_type::get<cl_mem_flags>(id, CL_MEM_FLAGS);
  }
  ...
  cl_uint mapcount() const {
    return info_type::get<cl_uint>(id, CL_MEM_MAP_COUNT);
  }
  cl_mem parent() const {
    return info_type::get<cl_mem>(id, CL_MEM_ASSOCIATED_MEMOBJECT);
  }
...
struct kernel_t {
  ...
  kernel_t(const kernel_t& kern) : id(0) { *this = kern; }

  kernel_t(cl_kernel _id) : id(_id) { clRetainKernel(id); }
  ...
  cl_uint refcount() const {
    return info_type::get<cl_uint>(id, CL_KERNEL_REFERENCE_COUNT);
  }
  std::string name() const {
    return info_type::getstr(id, CL_KERNEL_FUNCTION_NAME);
  }
  cl_uint num_args() const {
    return info_type::get<cl_uint>(id, CL_KERNEL_NUM_ARGS);
  }
  cl_context context() const {
    return info_type::get<cl_context>(id, CL_KERNEL_CONTEXT);
  }
  cl_program program() const {
    return info_type::get<cl_program>(id, CL_KERNEL_PROGRAM);
  }
...
struct program_t {
  ...
  program_t(const program_t& prog) : id(0) { *this = prog; }

  program_t(cl_program _id) : id(_id) { clRetainProgram(id); }
  ...
  cl_uint refcount() const {
    return info_type::get<cl_uint>(id, CL_CONTEXT_REFERENCE_COUNT);
  }
  cl_context context() const {
    return info_type::get<cl_context>(id, CL_PROGRAM_CONTEXT);
  }
  std::string source() const {
    return info_type::getstr(id, CL_PROGRAM_SOURCE);
  }
  cl_uint num_devices() const {
    return info_type::get<cl_uint>(id, CL_CONTEXT_NUM_DEVICES);
  }

  cl_build_status build_status(cl_device_id dev) const {
    return build_info_type::get<cl_build_status>(std::make_pair(id, dev),
                                                 CL_PROGRAM_BUILD_STATUS);
  }

  std::string build_options(cl_device_id dev) const {
    return build_info_type::getstr(std::make_pair(id, dev),
  ...
  std::vector<cl_device_id> devices() const {
    ...
    info_type::getvec<cl_device_id>(id, CL_CONTEXT_DEVICES, &dev[0],
                                    dev.size());
    return dev;
  }

  cl_device_id device(int idx) const { return devices()[idx]; }

  bool build(size_t n_devices,
             cl_device_id* devices,
             const std::string& opts = "") {
    cl_int err =
        clBuildProgram(id, n_devices, devices, opts.c_str(), NULL, NULL);
    switch (err) {
      case CL_SUCCESS:
        return true;
      case CL_BUILD_PROGRAM_FAILURE:
  ...
  bool build(const std::string& opts = "") { return build(0, NULL, opts); }
  ...
  ctx_t(cl_device_type device_type, platform_t* plat = NULL) {
    ...
    if (plat) {
      cl_context_properties props[] = {CL_CONTEXT_PLATFORM,
                                       (cl_context_properties)plat->id, 0};
      id = clCreateContextFromType(props, device_type, _notify, (void*)this,
                                   &err);
    } else {
      id = clCreateContextFromType(NULL, device_type, _notify, (void*)this,
                                   &err);
    }
    _check(err);
  }

  ctx_t(const ctx_t& ctx) : id(0) { *this = ctx; }

  ctx_t(cl_context _id) : id(_id) { clRetainContext(id); }
...
struct queue_t {
  ...
  queue_t(const queue_t& q) : id(0) { *this = q; }

  queue_t(cl_command_queue _id) : id(_id) { clRetainCommandQueue(id); }

  queue_t(cl_context ctx,
          cl_device_id dev,
  ...
  cl_uint refcount() const {
    return info_type::get<cl_uint>(id, CL_QUEUE_REFERENCE_COUNT);
  }
  cl_context context() const {
    return info_type::get<cl_context>(id, CL_QUEUE_CONTEXT);
  }
  cl_device_id device() const {
    return info_type::get<cl_device_id>(id, CL_QUEUE_DEVICE);
  }
...
struct opencl_t {
  ...
  void init() {
    ...
    for (cl_uint i = 0; i < n_plat; ++i)
      cl_platforms.push_back(new platform_t(plat[i]));
    delete[] plat;
  }
  ...
  void list(std::ostream& out) const {
    for (size_t i = 0; i < cl_platforms.size(); ++i) {
      ...
      out << plat.id << " " << plat.name() << " (" << plat.profile()
          << " ver: " << plat.version() << " vendor: " << plat.vendor() << ")"
          << std::endl;

      for (size_t j = 0; j < plat.cl_devices.size(); ++j) {
        const dev_t& dev = *plat.cl_devices[j];
        out << "\t" << dev.id << " " << dev.name() << " (" << dev.profile()
            << " ver: " << dev.version() << " vendor: " << dev.vendor() << ")"
            << std::endl;
      }
    }
  }
