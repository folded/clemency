...
class m4_t {
 public:
  union {
    ...
    // .mCR
    struct {
      num_t m00, m01, m02, m03, m10, m11, m12, m13, m20, m21, m22, m23, m30,
          m31, m32, m33;
    };
    // m[C][R]
    num_t m[4][4];
  ...
  // note: arguments are provided in the natural orientation, but stored
  // transposed.
  static m4_t init(num_t m00,
                   num_t m10,
                   num_t m20,
                   num_t m30,
                   num_t m01,
  ...
  static m4_t rot(const quat_t<num_t>& q) {
    ...
    return m4_t::init(num_t(1) - num_t(2) * y * y - num_t(2) * z * z,
                      num_t(2) * x * y - num_t(2) * z * w,
                      num_t(2) * x * z + num_t(2) * y * w, num_t(0),
                      num_t(2) * x * y + num_t(2) * z * w,
                      num_t(1) - num_t(2) * x * x - num_t(2) * z * z,
                      num_t(2) * y * z - num_t(2) * x * w, num_t(0),
                      num_t(2) * x * z - num_t(2) * y * w,
                      num_t(2) * y * z + num_t(2) * x * w,
                      num_t(1) - num_t(2) * x * x - num_t(2) * y * y, num_t(0),
  ...
  num_t operator()(int col, int row) const { return m[col][row]; }

  num_t& operator()(int col, int row) { return m[col][row]; }
