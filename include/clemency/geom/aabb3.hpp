...
class aabb3_t {
 public:
  ...
  static aabb3_t initWithPoints(const v3_t<num_t>& a,
                                const v3_t<num_t>& b,
                                const v3_t<num_t>& c) {
    aabb3_t result;
  ...
  num_t volume() const { return (xh() - xl()) * (yh() - yl()) * (zh() - zl()); }

  v3_t<num_t> corner(unsigned c) const {
    return v3_t<num_t>::init(
        mid.x + extent.x * (c & 1 ? num_t(+1) : num_t(-1)),
  ...
  bool touchesFace(const aabb3_t& bbox) const {
    ...
    return (dx == num_t(0) && dy < num_t(0) && dz < num_t(0)) ||
  ...
  bool contains(const v3_t<num_t>& p) const {
    return std::abs(p.x - mid.x) < extent.x &&
           std::abs(p.y - mid.y) < extent.y && std::abs(p.z - mid.z) < extent.z;
  }
  ...
  bool contains(const sphere_t<num_t>& sp) const {
    return std::abs(sp.v.x - mid.x) < extent.x - sp.r &&
  ...
  bool intersects(const sphere_t<num_t>& sp) const {
    ...
    for (unsigned i = 0; i < 3; ++i) {
      num_t t = std::abs(sp.v.v[i] - mid.v[i]) - extent.v[i];
      if (t > num_t(0))
        r += t * t;
    }
    return r <= sp.r * sp.r;
  ...
  num_t distance(const v3_t<num_t>& p) const { return sqrt(distancesq(p)); }
};

template <typename num_t>
std::ostream& operator<<(std::ostream& out, const aabb3_t<num_t>& aabb) {
  out << aabb.xl() << "," << aabb.yl() << "," << aabb.zl() << ":" << aabb.xh()
      << "," << aabb.yh() << "," << aabb.zh();
  return out;
}

template <typename rng_t, typename num_t>
std::pair<v3_t<num_t>, v3_t<num_t> > randomLineSegmentInAABB(
    rng_t& rng,
    const aabb3_t<num_t>& aabb) {
...
v3_t<num_t> randomOnAABB(rng_t& rng, const aabb3_t<num_t>& aabb) {
  ...
  boost::random::variate_generator<rng_t&, boost::uniform_01<num_t> > gen(
      rng, uniform);
  v3_t<num_t> pos;
  int f = face(rng);
...
bool intersectsTriangle_axisTest_3(const aabb3_t<num_t>& aabb,
                                   const tri3_t<num_t>& tri) {
  const int d = (c + 1) % 3, e = (c + 2) % 3;
  const v3_t<num_t> a =
      v3_t<num_t>::cross(v3_t<num_t>::init(Ax, Ay, Az), tri.v[d] - tri.v[c]);
...
bool intersectsTriangle_axisTest_2(const aabb3_t<num_t>& aabb,
                                   const tri3_t<num_t>& tri) {
...
template <typename num_t>
bool intersectsTriangle_axisTest_1(const aabb3_t<num_t>& aabb,
                                   const tri3_t<num_t>& tri) {
