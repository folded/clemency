...
struct v3_t {
  ...
  num_t operator[](int i) const { return v[i]; }

  num_t& operator[](int i) { return v[i]; }

  num_t length() const { return ::sqrt(dot(*this, *this)); }

  num_t lengthsq() const { return dot(*this, *this); }

  v3_t abs() const { return v3_t::init(std::abs(x), std::abs(y), std::abs(z)); }
  ...
  v3_t scale(num_t s) const { return init(x * s, y * s, z * s); }
  ...
  static v3_t neg(const v3_t& a) { return init(-a.x, -a.y, -a.z); }
  ...
  static v3_t cross(const v3_t& a, const v3_t& b) {
    return init(+(a.y * b.z - b.y * a.z), -(a.x * b.z - b.x * a.z),
                +(a.x * b.y - b.x * a.y));
  }
  ...
  static bool neq(const v3_t& a, const v3_t& b) { return !eq(a, b); }
  ...
  //         -ve = d is above a,b,c
  //           0 = d is on a,b,c
  static num_t orient(const v3_t& a,
                      const v3_t& b,
                      const v3_t& c,
                      const v3_t& d);
  ...
  static num_t tetrahedron_volume(const v3_t& a,
                                  const v3_t& b,
                                  const v3_t& c,
                                  const v3_t& d);
};
...
template <typename num_t>
num_t v3_t<num_t>::dotcross(const v3_t<num_t>& a,
                            const v3_t<num_t>& b,
                            const v3_t<num_t>& c) {
