...
class octree_t {
 public:
  ...
  bool is_leaf() const { return children[0] == NULL; }

  v3_t<num_t> corner(unsigned c) const { return bbox.corner(c); }

  void split(const v3_t<num_t>& expand = v3_t<num_t>::zero()) {
    if (children[0])
      throw std::runtime_error("node already split");

    v3_t<num_t> lo = bbox.mid - bbox.extent;
    v3_t<num_t> hi = bbox.mid + bbox.extent;
    v3_t<num_t> md = bbox.mid;

    children[0] = new child_t(aabb3_t<num_t>::initWithPoints(
        v3_t<num_t>::init(lo.x, lo.y, lo.z) - expand,
        v3_t<num_t>::init(md.x, md.y, md.z) + expand));
    children[1] = new child_t(aabb3_t<num_t>::initWithPoints(
        v3_t<num_t>::init(md.x, lo.y, lo.z) - expand,
        v3_t<num_t>::init(hi.x, md.y, md.z) + expand));
    children[2] = new child_t(aabb3_t<num_t>::initWithPoints(
        v3_t<num_t>::init(lo.x, md.y, lo.z) - expand,
        v3_t<num_t>::init(md.x, hi.y, md.z) + expand));
    children[3] = new child_t(aabb3_t<num_t>::initWithPoints(
        v3_t<num_t>::init(md.x, md.y, lo.z) - expand,
        v3_t<num_t>::init(hi.x, hi.y, md.z) + expand));

    children[4] = new child_t(aabb3_t<num_t>::initWithPoints(
        v3_t<num_t>::init(lo.x, lo.y, md.z) - expand,
        v3_t<num_t>::init(md.x, md.y, hi.z) + expand));
    children[5] = new child_t(aabb3_t<num_t>::initWithPoints(
        v3_t<num_t>::init(md.x, lo.y, md.z) - expand,
        v3_t<num_t>::init(hi.x, md.y, hi.z) + expand));
    children[6] = new child_t(aabb3_t<num_t>::initWithPoints(
        v3_t<num_t>::init(lo.x, md.y, md.z) - expand,
        v3_t<num_t>::init(md.x, hi.y, hi.z) + expand));
    children[7] = new child_t(aabb3_t<num_t>::initWithPoints(
        v3_t<num_t>::init(md.x, md.y, md.z) - expand,
        v3_t<num_t>::init(hi.x, hi.y, hi.z) + expand));
  }
...
class octree_adjacent_cells_t {
  ...
  void emit(node_t* q0, iter_t& out) const {
    std::vector<node_t*> r;
    r.resize(1);
    r[0] = q0;
    *out++ = r;
  ...
  template <typename iter_t>
  void emit(node_t* q0, node_t* q1, iter_t& out) const {
    std::vector<node_t*> r;
    r.resize(2);
    r[0] = q0;
    r[1] = q1;
  ...
  template <typename iter_t>
  void emit(node_t* q0, node_t* q1, node_t* q2, node_t* q3, iter_t& out) const {
    std::vector<node_t*> r;
    r.resize(4);
    r[0] = q0;
    r[1] = q1;
  ...
  void dual_3(node_t* q0, iter_t& out) const {
    ...
    if (lev == 0) {
      dual_0<lev>(ch(q0, 0), ch(q0, 1), ch(q0, 2), ch(q0, 3), ch(q0, 4),
                  ch(q0, 5), ch(q0, 6), ch(q0, 7), out);
    }
  }
  ...
  void dual_2x(node_t* q0, node_t* q1, iter_t& out) const {
    ...
    if (lev == 0) {
      dual_0<lev>(ch(q0, 1), ch(q1, 0), ch(q0, 3), ch(q1, 2), ch(q0, 5),
                  ch(q1, 4), ch(q0, 7), ch(q1, 6), out);
    }
  }
  ...
  void dual_2y(node_t* q0, node_t* q1, iter_t& out) const {
    ...
    if (lev == 0) {
      dual_0<lev>(ch(q0, 2), ch(q0, 3), ch(q1, 0), ch(q1, 1), ch(q0, 6),
                  ch(q0, 7), ch(q1, 4), ch(q1, 5), out);
    }
  }
  ...
  void dual_2z(node_t* q0, node_t* q1, iter_t& out) const {
    ...
    if (lev == 0) {
      dual_0<lev>(ch(q0, 4), ch(q0, 5), ch(q0, 6), ch(q0, 7), ch(q1, 0),
                  ch(q1, 1), ch(q1, 2), ch(q1, 3), out);
    }
  }

  // 4 args - edge, meeting at a line y=i, z=j
  template <int lev, typename iter_t>
  void dual_1x(node_t* q0, node_t* q1, node_t* q2, node_t* q3, iter_t& out)
      const {
    if (is_leaf(q0) && is_leaf(q1) && is_leaf(q2) && is_leaf(q3)) {
      if (lev == 1)
        emit(q0, q1, q2, q3, out);
      return;
    }
    ...
    if (lev == 0) {
      dual_0<lev>(ch(q0, 6), ch(q0, 7), ch(q1, 4), ch(q1, 5), ch(q2, 2),
                  ch(q2, 3), ch(q3, 0), ch(q3, 1), out);
    }
  }

  // 4 args - edge, meeting at a line x=i, z=j
  template <int lev, typename iter_t>
  void dual_1y(node_t* q0, node_t* q1, node_t* q2, node_t* q3, iter_t& out)
      const {
    if (is_leaf(q0) && is_leaf(q1) && is_leaf(q2) && is_leaf(q3)) {
      if (lev == 1)
        emit(q0, q1, q2, q3, out);
    ...
    if (lev == 0) {
      dual_0<lev>(ch(q0, 5), ch(q1, 4), ch(q0, 7), ch(q1, 6), ch(q2, 1),
                  ch(q3, 0), ch(q2, 3), ch(q3, 2), out);
    }
  }

  // 4 args - edge, meeting at a line x=i, y=j
  template <int lev, typename iter_t>
  void dual_1z(node_t* q0, node_t* q1, node_t* q2, node_t* q3, iter_t& out)
      const {
    if (is_leaf(q0) && is_leaf(q1) && is_leaf(q2) && is_leaf(q3)) {
      if (lev == 1)
        emit(q0, q1, q2, q3, out);
    ...
    if (lev == 0) {
      dual_0<lev>(ch(q0, 3), ch(q1, 2), ch(q2, 1), ch(q3, 0), ch(q0, 7),
                  ch(q1, 6), ch(q2, 5), ch(q3, 4), out);
    }
  }

  // 8 args - vertex
  template <int lev, typename iter_t>
  void dual_0(node_t* q0,
              node_t* q1,
              node_t* q2,
              node_t* q3,
              node_t* q4,
