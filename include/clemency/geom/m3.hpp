...
class m3_t {
 public:
  union {
    // .mCR
    struct {
      num_t m00, m01, m02, m10, m11, m12, m20, m21, m22;
    };
    // m[C][R]
    num_t m[3][3];
    // v[C*3+R]
  ...
  static m3_t rot(const quat_t<num_t>& q) {
    ...
    return m3_t::init(num_t(1) - num_t(2) * y * y - num_t(2) * z * z,
                      ...
                      num_t(2) * x * z - num_t(2) * y * w,
                      num_t(2) * y * z + num_t(2) * x * w,
                      num_t(1) - num_t(2) * x * x - num_t(2) * y * y);
  }
  ...
  num_t operator()(int i, int j) const { return m[i][j]; }

  num_t& operator()(int i, int j) { return m[i][j]; }

  num_t mdet(int i, int j) const {
    return +m[i == 0 ? 1 : 0][j == 0 ? 1 : 0] *
               m[i == 2 ? 1 : 2][j == 2 ? 1 : 2] -
           m[i == 2 ? 1 : 2][j == 0 ? 1 : 0] *
               m[i == 0 ? 1 : 0][j == 2 ? 1 : 2];
  }
