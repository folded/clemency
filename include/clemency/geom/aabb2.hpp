...
class aabb2_t {
 public:
  ...
  static aabb2_t initWithPoints(const v2_t<num_t>& a,
                                const v2_t<num_t>& b,
                                const v2_t<num_t>& c) {
    aabb2_t result;
  ...
  num_t area() const { return (xh() - xl()) * (yh() - yl()); }

  v2_t<num_t> corner(unsigned c) const {
    return v2_t<num_t>::init(
  ...
  bool contains(const v2_t<num_t>& p) const {
    return std::abs(p.x - mid.x) < extent.x && std::abs(p.y - mid.y) < extent.y;
  }

  bool intersects(const v2_t<num_t>& p) const { return contains(p); }
  ...
  num_t distance(const v2_t<num_t>& p) const { return sqrt(distancesq(p)); }
};
