...
class tri_base_t {
 ...
 public:
  ...
  tri_base_t(size_t _a, size_t _b, size_t _c)
      : tag(0),
        next(static_cast<tri_t*>(this)),
        prev(static_cast<tri_t*>(this)),
        a(_a),
        b(_b),
        c(_c),
        va(NULL),
  ...
  v3d_t normal(const vlookup_t& v) const {
    ...
    if (pa == pb || pa == pc || pb == pc) {
      std::cerr << "warning: calculating normal of degenerate triangle"
                << std::endl;
      return v3d_t::zero();
    }
...
class vert_base_t {
 public:
  ...
  void copy(const vert_base_t& v) { pos = v.pos; }
};
...
class vert_t : public vert_base_t<vert_t, tri_t> {
 ...
 public:
  ...
  void copy(const vert_t& v) { super_t::copy(v); }
};
...
struct triangle_mesh_t {
  ...
  struct const_face_iter {
    ...
    reference operator*() const { return *node; }
    pointer operator->() const { return node; }
    const_face_iter& operator++() {
      node = node->next;
      return *this;
    }
    const_face_iter operator++(int) {
      const_face_iter tmp = *this;
      node = node->next;
      return tmp;
    }
    const_face_iter& operator--() {
    ...
    bool operator==(const const_face_iter& other) const {
  ...
  triangle_mesh_t()
      : curr_tag(0), face_list(0, 0, 0), face_count(0), vertices() {}

  ~triangle_mesh_t() { clear_tris(); }
  ...
  struct vlookup_t {
    ...
    const v3d_t& operator()(const size_t& v) const { return vertices[v].pos; }
  };

  vlookup_t vertex_getter() const { return vlookup_t(vertices); }

  aabb3d_t aabb(const tri_t& t) const { return t.aabb(vlookup_t(vertices)); }

  v3d_t normal(const tri_t& t) const { return t.normal(vlookup_t(vertices)); }

  v3d_t centroid(const tri_t& t) { return t.centroid(vlookup_t(vertices)); }
  ...
  tri_t* remove_tri(tri_t* t) {
    ...
    // std::cerr << "remove tri " << t << " ; " << t->a << "," << t->b << "," <<
    // t->c << std::endl;
    // std::cerr << "  a "; for (tri_t *t2 = vertices[t->a].face; t2 != NULL;
    // vnext(t2, t->a)) std::cerr << " " << t2; std::cerr << std::endl;
    // std::cerr << "  b "; for (tri_t *t2 = vertices[t->b].face; t2 != NULL;
    // vnext(t2, t->b)) std::cerr << " " << t2; std::cerr << std::endl;
    // std::cerr << "  c "; for (tri_t *t2 = vertices[t->c].face; t2 != NULL;
    // vnext(t2, t->c)) std::cerr << " " << t2; std::cerr << std::endl;
  ...
  void collapse_edge(size_t a, size_t b) {
    ...
    // {
    //   std::cerr << "in triangles incident to " << a << " = " <<
    //   vertex_face_count(a) << std::endl;
    //   tri_t *t = vertices[a].face;
    //   while (t) {
    //     std::cerr << "    -> " << t << " " << t->a << ", " << t->b << ", " <<
    //     t->c << std::endl;
    ...
    // }
    // {
    //   std::cerr << "in triangles incident to " << b << " = " <<
    //   vertex_face_count(b) << std::endl;
    ...
    if (!vertices[a].face || !vertices[b].face)
      return;
  ...
  void get_vertex_triangles(size_t a, iter_t iter) {
    for (tri_t* t = vertices[a].face; t; vnext(t, a))
      *iter++ = t;
  }
  ...
  double _volume(iter_t begin, iter_t end, tri_t) const {
    ...
    for (; begin != end; ++begin) {
      tri_t& t = *begin;
      vol += v3d_t::tetrahedron_volume(vertices[t.a].pos, vertices[t.b].pos,
                                       vertices[t.c].pos, v3d_t::zero());
    }
    return vol;
  }
  ...
  double volume(iter_t begin, iter_t end) const {
    return _volume(begin, end,
                   typename std::iterator_traits<iter_t>::value_type());
  }

  double volume() const { return volume(fbegin(), fend()); }
  ...
  v3d_t _centre_of_mass(iter_t begin, iter_t end, tri_t) const {
    ...
    for (; begin != end; ++begin) {
      tri_t& t = *begin;
      double tvol =
          v3d_t::tetrahedron_volume(vertices[t.a].pos, vertices[t.b].pos,
                                    vertices[t.c].pos, v3d_t::zero());
    ...
    for (; begin != end; ++begin) {
      ...
      double tvol =
          v3d_t::tetrahedron_volume(vertices[t->a].pos, vertices[t->b].pos,
                                    vertices[t->c].pos, v3d_t::zero());
      com +=
          tvol * (vertices[t->a].pos + vertices[t->b].pos + vertices[t->c].pos);
      vol += tvol;
    }
    return com / (vol * 4.0);
  }
  ...
  v3d_t centre_of_mass(iter_t begin, iter_t end) const {
    return _centre_of_mass(begin, end,
                           typename std::iterator_traits<iter_t>::value_type());
  }

  v3d_t centre_of_mass() const { return centre_of_mass(fbegin(), fend()); }

  bool is_closed_manifold() const;

  template <typename iter_t>
  static tri_t* _tri_ptr(iter_t iter, tri_t) {
    return &*iter;
  }
  ...
  triangle_mesh_t* submesh(iter_t begin, iter_t end) const {
    ...
    for (iter_t i = begin; i != end; ++i) {
      ...
      for (size_t j = 0; j < 3; ++j) {
        std::unordered_map<size_t, size_t>::iterator iter =
            vertex_map.find(t->v[j]);
...
mesh_t* merge_meshes(mesh_t* a, mesh_t* b) {
  typedef std::unordered_map<typename mesh_t::vert_t, size_t, vert_hash_t,
                             vert_eq_t>
      vert_map_t;
  vert_map_t vert_map;
  ...
  for (typename mesh_t::tri_t* t = a->face_list.next; t != &a->face_list;
       t = t->next) {
    result->add_tri(a_remap[t->a], a_remap[t->b], a_remap[t->c]);
  }

  for (typename mesh_t::tri_t* t = b->face_list.next; t != &b->face_list;
       t = t->next) {
...
bool triangle_mesh_t<_vert_t, _tri_t>::is_closed_manifold() const {
  typedef std::unordered_map<std::pair<size_t, size_t>, std::vector<size_t>,
                             hash_size_t_pair>
      edge_map_t;
  ...
  for (typename edge_map_t::iterator i = edge_map.begin(); i != edge_map.end();
       ++i) {
    typename edge_map_t::iterator j =
        edge_map.find(std::make_pair((*i).first.second, (*i).first.first));
    if (j == edge_map.end() || (*i).second.size() != 1 ||
        (*j).second.size() != 1) {
  ...
  std::cerr << "connected component count: " << connected_components.count()
            << std::endl;
  ...
  for (size_t i = 0; i < face_groups.size(); ++i) {
    ...
    std::cerr << "  component " << i << "(" << face_groups[i].size()
              << " faces) ";
    if (bf.size()) {
      std::cerr << "not closed" << std::endl;
    } else {
      volint_t<triangle_mesh_t<_vert_t, _tri_t> > vol(
          this, face_groups[i].begin(), face_groups[i].end());
      std::cerr << "closed"
                << " volume="
                << volume(face_groups[i].begin(), face_groups[i].end())
                << " com="
                << centre_of_mass(face_groups[i].begin(), face_groups[i].end())
                << " volume(2)=" << vol.volume()
                << " com(2)=" << vol.centre_of_mass() << std::endl;
    }
  }
  return true;
}
