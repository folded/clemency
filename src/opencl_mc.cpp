...
struct marching_cubes_t {
  ...
  marching_cubes_t() : px(), py(), pz(), int_idx(), mesh(NULL) {}
...
void validate(boost::any& v,
  ...
  if (sscanf(s.c_str(), "%lg,%lg,%lg:%lg,%lg,%lg", &xl, &yl, &zl, &xh, &yh,
             &zh) != 6) {
    throw opt::validation_error(opt::validation_error::invalid_option_value);
  }
  v = boost::any(aabb3d_t::initWithPoints(v3d_t::init(xl, yl, zl),
                                          v3d_t::init(xh, yh, zh)));
}
...
int main(int argc, char** argv) {
  ...
  std::string dst_file;
  aabb3d_t aabb;
  aabb3d_t aabb0 = aabb3d_t::initWithPoints(v3d_t::init(-1.0, -1.0, -1.0),
                                            v3d_t::init(+1.0, +1.0, +1.0));

  opt::options_description generic;
  generic.add_options()("help,h", "Print help and exit")(
      "version,v", "Print version string")("cpu,c", "Execute on CPU")(
      "list,l", "List available OpenCL devices")(
      "cl-opts,C", opt::value<std::vector<std::string> >(),
      "Set OpenCL compiler option")(
      "grid,g", opt::value<size_t>(&grid)->default_value(256),
      "Size of marching cubes grid")(
      "gx", opt::value<size_t>(&grid_x)->default_value(0),
      "X dimension of mc grid")("gy",
                                opt::value<size_t>(&grid_y)->default_value(0),
                                "Y dimension of mc grid")(
      "gz", opt::value<size_t>(&grid_z)->default_value(0),
      "Z dimension of mc grid")(
      "aabb,a", opt::value<aabb3d_t>(&aabb)->default_value(aabb0),
      "AABB of marching cubes grid")(
      "block0", opt::value<size_t>(&block0)->default_value(256),
      "Block size for grid evaluation")(
      "block1", opt::value<size_t>(&block1)->default_value(1024),
      "Block size for intersection refinement")(
      "refine,r", opt::value<size_t>(&refine)->default_value(10),
      "Number of refinement steps")(
      "output,o", opt::value<std::string>(&dst_file)->default_value("-"),
      "Ouput file (- for stdout)");

  opt::options_description hidden;
  hidden.add_options()("kernel", opt::value<std::string>(&src_file),
                       "OpenCL kernel to load");
  ...
  try {
    ...
    if (dst_file == "-") {
      ...
      if (!outf.good()) {
        std::cerr << "could not open " << dst_file << " for writing"
                  << std::endl;
        exit(1);
      }
