...
class scene_t : public dist_t {
 public:
  virtual double dist(v3d_t v) const {
    return smin_exp(v3d_t::sub(v, v3d_t::init(-.5, 0, .3)).length() - 1.0,
                    v3d_t::sub(v, v3d_t::init(0, .9, 0)).length() - 1.0, 30.0);
  }
...
class isosurface_octree_t : public octree_t<isosurface_octree_t, double> {
 public:
  ...
  isosurface_octree_t(const aabb3d_t& _bbox)
      : super(_bbox), dual_info(NULL), depth(0) {}
...
struct isosurface_splitter_t : public isosurface_octree_t::visitor_t {
  ...
  void split(isosurface_octree_t* root) { root->visit(*this); }
  ...
  void pre(isosurface_octree_t* node, int depth) {
    ...
    if (std::abs(d) > node->bbox.extent.length())
      return;
...
struct cell_t {
  node_t* cell;
  int pos_idx;
  enum cell_type_t { VERTEX, EDGE, FACE, VOL } cell_type;
  ...
  struct eq {
    bool operator()(const cell_t& a, const cell_t& b) const {
      return a.cell == b.cell && a.cell_type == b.cell_type &&
             a.pos_idx == b.pos_idx;
    }
  ...
  static cell_t min(iter_t a, iter_t b) {
    return *std::min_element(a, b, lt());
  }

  static cell_t min(const cell_t& a, const cell_t& b) {
    return lt()(a, b) ? a : b;
  }
  ...
  static void generate_tetrahedra(
      const std::vector<node_t*>& dc,
      std::back_insert_iterator<std::list<std::vector<cell_t> > > out) {
    cell_t cell0;

    cell0 =
        min(cell_t(dc[0], cell_t::VERTEX, 7), cell_t(dc[1], cell_t::VERTEX, 6),
            cell_t(dc[2], cell_t::VERTEX, 5), cell_t(dc[3], cell_t::VERTEX, 4),
            cell_t(dc[4], cell_t::VERTEX, 3), cell_t(dc[5], cell_t::VERTEX, 2),
            cell_t(dc[6], cell_t::VERTEX, 1), cell_t(dc[7], cell_t::VERTEX, 0));
...
struct result_t {
  ...
  typedef std::unordered_map<cell_type,
                             position_t,
                             typename cell_type::hash,
                             typename cell_type::eq>
      tvert_info_map_t;
  tvert_info_map_t tvert_info;

  typedef std::unordered_map<std::pair<cell_type, cell_type>,
                             size_t,
                             typename cell_type::hash,
                             typename cell_type::eq>
      int_idx_map_t;
  ...
  void dump(std::ostream& out) {
    ...
    for (size_t i = 0; i < int_pos.size(); ++i) {
      out << int_pos[i].x << " " << int_pos[i].y << " " << int_pos[i].z
          << std::endl;
    }
    for (size_t i = 0; i < tris.size(); ++i) {
      out << "3 " << tris[i].a << " " << tris[i].b << " " << tris[i].c
          << std::endl;
    }
  }
  ...
  position_t choose_position(cell_type cell) {
    ...
    switch (cell.cell_type) {
      case cell_type::VERTEX: {
        ...
        result.orient =
            result.dist <= OFFSET ? position_t::IN : position_t::OUT;
        break;
      }
      ...
      case cell_type::EDGE: {
        ...
        r.v[b] += cell.pos_idx & 1 ? +cell.cell->bbox.extent.v[b]
                                   : -cell.cell->bbox.extent.v[b];
        r.v[c] += cell.pos_idx & 2 ? +cell.cell->bbox.extent.v[c]
                                   : -cell.cell->bbox.extent.v[c];
      ...
      case cell_type::FACE: {
        ...
        r.v[a] += cell.pos_idx & 1 ? +cell.cell->bbox.extent.v[a]
                                   : -cell.cell->bbox.extent.v[a];
  ...
  void generate(const std::list<std::vector<cell_type> >& tetrahedra) {
    for (typename std::list<std::vector<cell_type> >::const_iterator i =
             tetrahedra.begin();
         i != tetrahedra.end(); ++i) {
      generate_one(*i);
    }
...
int main(int argc, char** argv) {
  ...
  //   Should the error just be the error at the minimum, or at the minimum
  //   subject to the constraints of the octree node?
  // position points in m-cells better to reduce triangle count.
  //   this is a matter of picking a cell of lower degree with opposite sign,
  //   and recursively subdividing to find a point on the surface.
  //   suggests that we need to assign locations to all cells of lower degree
  //   before the current degree.
  //     but would like to avoid doing this, because it would be good to be able
  //     to handle in parallel.
  //   a point on a vertex (0-cell) is completely fixed.
  //   a point on an edge (1-cell) is fixed to lie between its adjacent 0-cell
  //   points
  //     because we are only interested in minimal 1-cells, only 2 0-cells will
  //     ever be important.
  //   a point on a face (2-cell) is constrained by its adjacent 1-cells.
  //     this is more complex because a minimal 2-cell might be adjacent to
  //     smaller 2-cells, and thus more than 4 1-cells may border it.
  //   a point in a box (3-cell) is constrained by its adjacent 2-cells
  //     as with 2-cells, there may be more than 6 adjacent 2-cells
  //   repositioning points on the surface (for the 2-cell and 3-cell case) can
  //   affect topology in some cases. ignore until an example is seen?

  isosurface_octree_t pt(aabb3d_t::initWithPoints(
      v3d_t::init(-2.0, -2.0, -2.0), v3d_t::init(+2.0, +2.0, +2.0)));
  ...
  int q = 0;
  for (std::list<std::vector<isosurface_octree_t*> >::iterator i =
           dual_cells.begin();
       i != dual_cells.end(); ++i) {
    cell_t<isosurface_octree_t>::generate_tetrahedra(
        *i, std::back_inserter(tetrahedra));
  }
